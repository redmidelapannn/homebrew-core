...
class QtAT59 < Formula
  ...
  def install
    # ninja doesn't like @ in the buildpath directory name
    mktemp "QtAT59" do
      Pathname.pwd.install buildpath.children
      ...
      if build.with? "mysql"
        ...
        (Pathname.pwd/"brew_shim/mysql_config").write <<~EOS
          #!/bin/sh
          if [ x"$1" = x"--libs" ]; then
        ...
        args << "-mysql_config" << Pathname.pwd/"brew_shim/mysql_config"
      end
