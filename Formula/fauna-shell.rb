...
class FaunaShell < Formula
  ...
  test do
    ...
    o, e, s = Open3.capture3("#{bin}/fauna list-endpoints")
    assert_equal false, s.success?

    o, e, s = Open3.capture3("#{bin}/fauna add-endpoint https://endpoint1:8443", :stdin_data=>"secret\nendpoint1\n")
    assert s.success?
    ...
    o, e, s = Open3.capture3("#{bin}/fauna add-endpoint https://endpoint2:8443", :stdin_data=>"secret\n\n")
    assert s.success?
    ...
    o, e, s = Open3.capture3("#{bin}/fauna default-endpoint endpoint2")
    assert s.success?
    ...
    o, e, s = Open3.capture3("#{bin}/fauna delete-endpoint endpoint1", :stdin_data=>"y\n")
    assert s.success?
    ...
    o, e, s = Open3.capture3("#{bin}/fauna delete-endpoint endpoint2", :stdin_data=>"y\n")
    assert s.success?
